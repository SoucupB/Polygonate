...
#include "../cmk/cmocka.h"
#include "../cmk/cmocka_private.h"
...
static void fail_if_blocks_allocated(const ListNode * const check_point,
    ...
    if (allocated_blocks > 0) {
        ...
        cm_print_error("ERROR: %s leaked %" PRIuSIZE " block(s)\n", test_name,
                       allocated_blocks);
        exit_test(1);
    }
}
...
{
    print_message("[==========] %s: Running %" PRIuSIZE " test(s).\n",
                  group_name,
                  num_tests);
}
                                           ...
                                           struct CMUnitTestState *cm_tests)
{
    ...
    print_message("[==========] %s: %" PRIuSIZE " test(s) run.\n",
                  group_name,
                  total_executed);
    ...
    if (total_skipped) {
        print_error("[  SKIPPED ] %s: %" PRIuSIZE " test(s), listed below:\n",
                    group_name,
                    total_skipped);
        for (i = 0; i < total_executed; i++) {
            struct CMUnitTestState *cmtest = &cm_tests[i];
        ...
        print_error("\n %" PRIuSIZE " SKIPPED TEST(S)\n", total_skipped);
    }

    if (total_failed) {
        print_error("[  FAILED  ] %s: %" PRIuSIZE " test(s), listed below:\n",
                    group_name,
                    total_failed);
        for (i = 0; i < total_executed; i++) {
            struct CMUnitTestState *cmtest = &cm_tests[i];
